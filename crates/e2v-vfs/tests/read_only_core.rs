...
fn windows_mount_launcher_can_hand_request_and_context_to_a_winfsp_host() {
    ...
    let launcher = WindowsMountLauncher::from_test_request(&request);
    let context = WinfspMountContext::from_test_request(request).unwrap();
...
fn winfsp_mount_context_captures_mount_request_and_cache_policy() {
    ...
    let context = WinfspMountContext::from_test_request(request.clone()).unwrap();
...
fn winfsp_mount_context_refreshes_live_branch_and_reports_invalidation_need() {
    ...
    let branch_token = RepositoryFacade::new()
        ...
        .token_hex;
    let request = MountRequest::live_branch(repo_root.clone(), branch_token, PathBuf::from("M:"));
    let mut context = WinfspMountContext::from_test_request(request).unwrap();
...
#[cfg(windows)]
#[test]
fn windows_mount_context_reports_invalid_snapshot_request_without_panicking() {
    let temp = tempdir().unwrap();
    let repo_root = temp.path().join("repo");
    fs::create_dir_all(&repo_root).unwrap();
    init_repo(&repo_root);
    ...
    let request = MountRequest::from_config(
        VfsMountConfig::snapshot(repo_root, "missing-snapshot-id".to_string()),
        PathBuf::from("Z:"),
    );

    let result = WinfspMountContext::from_test_request(request);

    let error = result.expect_err("expected invalid snapshot mount request to return an error");
    assert!(
        error.to_string().contains("missing-snapshot-id")
